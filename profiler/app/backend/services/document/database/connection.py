...
This module provides functions to create and manage database connections for document service.
"""
...
class DatabaseManager:
    ...
    async def initialize(self) -> None:
        ...
        if self._initialized:
            logger.info("Document database already initialized, skipping")
            return
            
        try:
            logger.info("Initializing document database connection")
            ...
            logger.info(f"Connecting to document database: {self._sanitize_url(db_url)}")
            ...
            logger.info("Document database connection initialized successfully")
        except Exception as e:
            logger.exception(f"Failed to initialize document database: {str(e)}")
            raise
    ...
    async def shutdown(self) -> None:
        ...
        """
        if self._engine:
            logger.info("Shutting down document database connection")
